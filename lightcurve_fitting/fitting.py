...
class def_log():
    def __init__(self, lc, model, priors, use_sigma, sigma_type):
    ...
    def __call__(self, p):
        log_prior = 0.
        for prior, p_i in zip(self.priors, p):
            log_prior += prior(p_i)
        if np.isinf(log_prior):
            return log_prior
        log_likelihood = self.model.log_likelihood(self.lc, p, use_sigma=self.use_sigma, sigma_type=self.sigma_type)
...
def lightcurve_mcmc(lc, model, priors=None, p_min=None, p_max=None, p_lo=None, p_up=None,
    ...
    with mp.Pool(processes=min(mp.cpu_count(), nwalkers)) as pool:
        ...
        sampler = emcee.EnsembleSampler(nwalkers, ndim, def_log(lc=lc, model=model, priors=priors, use_sigma=use_sigma, sigma_type=sigma_type),
                                         pool=pool)
